...
print(vetor_total_resultados) 

### Barplot ####
?barplot
barplot(vetor_total_resultados, col = c(1,2,3,4,5)) ###parametro <col> possibilita personalizar cores. 

### Salvar gráfico em disco ####
png("barplot.png", width= 480, height= 480) #Cria o arquivo no diretório
barplot(vetor_total_resultados, 
        col = rgb(0.5, 0.1, 0.6, 0.6), 
        xlab = "Categoriaas", 
        ylab = "Valores", 
        main = "Barplot em R", 
        ylim = c(0,60) ### Limites do gráfico 
        )
dev.off() ## Salva em Disco

#ggplot2 
library(ggplot2) ###Carregar o pacote ggplot2 na sessão
View(mtcars)
